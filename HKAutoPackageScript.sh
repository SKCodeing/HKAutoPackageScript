...
# step2 : 打开终端, cd到HKAutoPackageScript文件夹
# step3 : 输入 sh HKAutoPackageScript.sh 命令,回车,开始执行打包脚本
...
pgyerUKey="*************"
pgyerApiKey="************"
pgyerDownloadUrl="https://www.pgyer.com/*****"
...
ExportOptionsPlistPath="./HKAutoPackageScript/AdHocExportOptionsPlist.plist"
# 返回上一级目录,进入项目工程目录
cd ..
# 获取项目名称
...
then
    ...
    ExportOptionsPlistPath="./HKAutoPackageScript/AppStoreExportOptionsPlist.plist"
    elif [ "$method" = "3" ] ; then
    ExportOptionsPlistPath="./HKAutoPackageScript/EnterpriseExportOptionsPlist.plist"
    elif [ "$method" = "4" ] ; then
    ExportOptionsPlistPath="./HKAutoPackageScript/DevelopmentExportOptionsPlist.plist"
    else
    echo "输入的参数无效!!!"
    exit 1
    fi
...
# 输出打包总用时
echo "\033[36;1m使用HKAutoPackageScript打包总用时: ${SECONDS}s \033[0m"
